...
class Network(base_layer.Layer):
  ...
  def call(self, inputs, training=None, mask=None):
    ...
    print("in def Network.call()")
    inputs = generic_utils.to_list(inputs)
    print("convert inputs to list:", inputs)
    if mask is None:
      masks = [None for _ in range(len(inputs))]
    else:
      masks = generic_utils.to_list(mask)
  ...
  def _run_internal_graph(self, inputs, training=None, mask=None):
    ...
    print("output tensor_map:", tensor_map)
    print("reference_output_tensors:", reference_output_tensors)
    print("output_tensors:", output_tensors)
    print("self.outputs:", self.outputs)
    print("str(id(self.outputs)):", str(id(self.outputs)))
    print(">>>>>>>>>>>>>\n")
    ...
    output_shapes = []
    for x in self.outputs:
      print("for checking of self.outputs in x:", x)
      print("for checking of self.outputs in str(id(x)):", str(id(x)))
